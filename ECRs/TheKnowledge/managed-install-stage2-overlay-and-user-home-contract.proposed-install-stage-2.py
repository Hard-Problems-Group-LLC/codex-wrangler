#!/usr/bin/env python3
"""Proposed managed stage-two installer with explicit user-home targeting.

This companion draft belongs to the ECR
`managed-install-stage2-overlay-and-user-home-contract.md`. It is intended as
a candidate TheKnowledge managed `scripts/install-stage-2.py` shape, not as a
repository-local script to execute from this ECR directory.
"""
...
AI_LOCAL_HOME_DIRNAMES = (".codex-home",)
...
def current_home() -> Path:
    """Return the active process home as an absolute path."""

    return Path.home().expanduser().resolve()


def default_user_bin_dir(user_home: Path) -> Path:
    """Return the managed user-local bin directory under one home."""

    return user_home.expanduser().resolve() / ".local" / "bin"


def default_user_share_dir(user_home: Path) -> Path:
    """Return the managed user-local share directory under one home."""
...
def default_target_pyenv_root(user_home: Path) -> Path:
    """Return the pyenv root for one resolved user-home target."""

    return user_home.expanduser().resolve() / ".pyenv"


def _path_is_relative_to(child: Path, parent: Path) -> bool:
    """Return True when `child` is located under `parent` on Python 3.9."""
...
def looks_like_project_local_ai_home(user_home: Path, repo_root: Path) -> bool:
    """Return True when `user_home` appears to be a project-local AI home."""

    resolved_home = user_home.expanduser().resolve()
    resolved_root = repo_root.expanduser().resolve()
    if not _path_is_relative_to(resolved_home, resolved_root):
        return False
    return resolved_home.name in AI_LOCAL_HOME_DIRNAMES
...
def describe_user_home_source(
    ...
    if looks_like_project_local_ai_home(user_home, repo_root):
        return "current project-local AI home"
...
def resolve_user_home(
    ...
    *,
    ...
    if (
        looks_like_project_local_ai_home(user_home, repo_root)
        and not allow_isolated_home
    ):
        raise RuntimeError(
            "Current HOME resolves to a project-local AI home ({}). Rerun "
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "isolated project-local AI home and you intend to target another "
            "user scope."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "project-local AI home when HOME isolation is active."
        ),
    )
    parser.add_argument(
        "--skip-direnv-install",
...
def ensure_runtime_contexts(user_home: Path) -> Tuple[Path, str, Path]:
    ...
    pyenv_root_path = default_target_pyenv_root(user_home)
...
def preferred_standard_base_python(
    ...
    candidate = pyenv_python_executable(
        default_target_pyenv_root(user_home),
...
def _pyproject_text(repo_root: Path) -> str:
    """Return `pyproject.toml` text when present."""
    ...
    try:
        ...
        if scope == REPO_SCOPE:
            if target_user_home is None:
                raise RuntimeError("Repo-scoped installs require a resolved user home.")
