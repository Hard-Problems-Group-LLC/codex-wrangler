...
def build_gitignore_block(layout: Layout) -> str:
    ...
    lines = [
        ...
        MAINTENANCE_LOCK_FILENAME,
        INITIAL_INSTALL_FILENAME,
        ".{}.*.tmp".format(INITIAL_INSTALL_FILENAME),
...
def build_maintenance_lock_gitignore_block() -> str:
    ...
    return "\n".join(
        [
            GITIGNORE_BEGIN,
            "# Stable lock and possible initialization-write debris retained after uninstall.",
            MAINTENANCE_LOCK_FILENAME,
            INITIAL_INSTALL_FILENAME,
            ".{}.*.tmp".format(INITIAL_INSTALL_FILENAME),
            GITIGNORE_END,
...
def build_launcher_preflight_lines() -> list[str]:
    ...
    return [
        ...
        "esac",
        "",
        "# Explicit CODEX_HOME must exist, but a missing protected HOME must not",
        "# be recreated. Initialize only this runtime-owned child, never parents.",
        'if [[ ! -d "$HOME" ]]; then',
        '  preflight_fail "Selected HOME directory is missing or not a directory: $HOME. Refusing to recreate it."',
        "fi",
        'if [[ "$effective_shared_home" != "1" && -L "$CODEX_HOME" ]]; then',
        '  preflight_fail "Isolated CODEX_HOME may not be a symbolic link: $CODEX_HOME"',
        "fi",
        'if [[ ! -d "$CODEX_HOME" ]]; then',
        '  if [[ -e "$CODEX_HOME" || -L "$CODEX_HOME" ]]; then',
        '    preflight_fail "CODEX_HOME is not a usable directory: $CODEX_HOME"',
        "  fi",
        '  if ! mkdir -m 700 -- "$CODEX_HOME" 2>/dev/null; then',
        "    # Another first launch may have created the same real directory.",
        '    if [[ ! -d "$CODEX_HOME" || -L "$CODEX_HOME" ]]; then',
        '      preflight_fail "Cannot create CODEX_HOME directory: $CODEX_HOME. Check HOME ownership and permissions."',
        "    fi",
        "  fi",
        "fi",
        "",
